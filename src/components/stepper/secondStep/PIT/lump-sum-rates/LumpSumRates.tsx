    ...
    return (
        <div className={`lump-sum-rates ${isOpen ? "open" : "close"}`}>
            <div ref={ref} className="lump-sum-box">
                ...
                <div className="rates">
                    {
                        rates.map(rate => <button aria-label={`ryczałt ${`${rate}`.replace(".", ",")}%`} onClick={() => { handleClick(rate) }} key={rate} className={`rate button button--chip ${lumpSum === rate ? "active" : ""}`}>{`${rate}`.replace(".", ",")}%</button>)
                    }
                </div>
            </div>
        </div>
